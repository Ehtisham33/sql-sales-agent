...
def login_proxy():
    ...
    if not auth_header or not auth_header.startswith("Bearer "): return jsonify({"error": "Authorization token required"}), 401
    token, data = auth_header.split(" ")[1], request.get_json()
    login_type, headers = data.get("LoginType"), {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    if login_type == "credentials":
        if not data.get("username") or not data.get("password"): return jsonify({"error": "Username and Password are required for POS login"}), 400
        login_url, payload = WMS_LOGIN_API_URL, {"username": data["username"], "password": data["password"]}
    else:
        if not data.get("EncryptedCred"): return jsonify({"error": "EncryptedCred is required for WMS login"}), 400
        login_url, payload = WMS_LOGIN_API_URL, {"EncryptedCred": data["EncryptedCred"]}
    try:
